...
for jj in range(5):
    ...
    net = Net(num_user, num_chan, 1000, 6).to(device)
    optimizer = torch.optim.Adam(net.parameters(), lr=2*1e-4)
    ...
    for index, (chan_val, chan_triu_val) in enumerate(test_dataloader):
        ...
        elapsed_time_PT = 1000 * (time.time() - cur_time) / tot_sample_te
        print("time = ", elapsed_time_PT)
        print("")
        ...
        cur_time = time.time()
        print(len(chan_triu_val))
        for ii in range(tot_sample_te):
            dl_sel = net(chan_triu_val[ii:ii+1].to(device))
            dl_per = 1
